...
void setup() {
  ...
  Serial.println(braille_version);
...
void practice_specialcharecter_reading_page(void) {
  ...
  while (true) {
    ...
    {
  char iv_str[2] = {0};
  getRandomStr(iv_str, 1,"special_char");
  Serial.print("random char "); Serial.println(iv_str);
  String random_kerneldata=userdata_to_kerneldata((String)iv_str);
  random_kerneldata=random_kerneldata.substring(1);
 if(disp_state){
...
void english_book_page() {
  ...
  while (true) {
    ...
    if (btn_accept_isdown) {
      String fname = "D/books/english_for_kids/" + (String)(sub_pos) + ".mp3";
...
void power_page() {
  ...
  while (true) {
    ...
    if (btn_accept_isdown) {
      switch (sub_pos) {
        case 1: Serial.println(F("power off "));return;
...
void getRandomStr(char* output, int len, String type){
  char* eligible_chars;
    if(type.compareTo("alp")== 0){eligible_chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";}
    if(type.compareTo("num")== 0){eligible_chars = "0123456789";}
    if(type.compareTo("special_char")== 0){eligible_chars = " !\\/#$%^&*()@_+-=[];:'\"<>?.,";}
    for(int i = 0; i< len; i++){
